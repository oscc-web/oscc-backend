...
export const consoleTransport = new transports.Console({
	format: format.combine(
		...
		format.printf(info => {
			const { level, label, duplicates, message, timestamp = new Date, ...args } = info,
		...
		info => {
			if (!cacheList.filter(c => c.check(info)).reduce((a, b) => a || b, false)) {
				cacheList.push(new CachedLog(info, dupLabel, c => {
					let index
					while ((index = cacheList.indexOf(c)) >= 0) cacheList.splice(index, 1)
				}))
			}
		},
		Object.fromEntries(
...
if (cluster.isPrimary) LogHub.addMessageHubListener(createCollapsedLog(`${PID}[*]`))
