...
export default class User extends CustomObject {
	/**
	 * User properties
	 ...
	 */
	#name
	get name() { return this.#name || this.#userID }
	set name(value) {
	...
	async viewGroups(targetUser) {
		// Treat any input other than User/GuestUser as TypeError
		...
		if (await this.hasPriv(PRIV.ALTER_USER_GROUP)) {
			...
			const sameUser = targetUser.userID == this.userID
			logger.debug(`Viewing groups of ${targetUser} as ${this}, same-user = ${sameUser}`)
			// Wait for this user's group to be instantiated
			const selfGroups = await this.groups
	...
	set password(value) {
		...
		this.#password = this.update({ $set: { password } }).then(successful => {
			return this.#password = !prev || successful
				? password
				: prev
		})
	}
	...
	async update(updateFilter) {
		...
		if (!updateFilter) {
			...
			const query = await User.db.user.find({ _id: this.userID }, { _id: true, name: true }).toArray()
			if (query.length === 0) {
				// Insert new user
				const result = await User.db.user.insert({ _id: this.userID, ...updateDoc })
				updated = result.acknowledged
			} else if (query.length === 1) {
				// Update existing user
				const result = await this.update({ $set: updateDoc })
			...
			const result = await User.db.user.update({ _id: this.userID }, updateFilter)
			updated = !!result.modifiedCount
		}
...
export class GuestUser extends CustomObject {
	get userID() { throw new TypeError }
	get mail() { throw new TypeError }
	get groups() {
		return Group.locate('guest').then(guest => [guest])
	}
	/**
	 * Get visible groups of current user from viewer's perspective
	 * @param {User} targetUser
	...
	async viewGroups(targetUser) {
		...
		// filter visible groups according to group.hasView
		return (await targetUser.groups).filter(
			group => group.hasView(false, ...selfGroups)
		)
	}
	// Object naming rules
	get [Symbol.toStringTag]() { return '@Guest' }
}
