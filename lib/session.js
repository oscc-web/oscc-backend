...
import dbInit from './mongo/mongo.js'
...
export default class Session {
	 ...
	 */
	async init() {
		if ('token' in this.#dsc) throw new Error('Session has already been initiated')
		// Generate unique session token, and store the session into database
		let result
		while (!result?.acknowledged) {
			result = await this.db.session.insert({
				token: seed(64),
				...this.#dsc
			})
		}
		this.#dsc.token = result.insertedId
		return this
	}
	/**
	...
	drop() {
		...
		Session.db.session.delete({ _id: this.#dsc?.token })
	}
	/**
	...
	static async preprocessor(req, res, next) {
		...
		const session = await this.locate(req)
		...
		req.injectCookies({ __internal_user_info__: await session.user.infoString })
	...
	static async locate(req) {
		...
		const [descriptor] = await this.db.session.find(
			{ _id: token },
			{ _id: false }
		).toArray()
