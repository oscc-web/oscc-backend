...
export class AppDataWithFs extends AppData {
	...
	async loadFile(identifier, duplicate = false) {
		const content = await fsAppData.load({ ...identifier, ...this.mark }, duplicate)
		// Check duplicate
		if (duplicate) {
	...
	static async registerFileUpload(
		...
		try {
			if (replace) {
				...
				await Promise.all(
					(await fsAppData.load({ userID, url }, true))
						.map(({ fileID, meta }) => new FileDescriptor(fileID, userID, meta))
						.map(fd => fd
							.delete()
