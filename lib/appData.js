...
export class AppData {
	...
	async store (content, identifier) {
		...
		let query = await AppData.db.appData.find({ identifier, appID: this.appID }, { _id: false, identifier: true }).toArray()
		let result = null
		if (query.length === 0) {
			// Insert new AppData
			let insertResult = await AppData.db.appData.insert({ content, identifier, appID: this.appID })
			result = insertResult
		} else {
