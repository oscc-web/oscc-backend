...
export default function privileged(
	...
	const privileges = Object.freeze((() => {
		if (!Array.isArray(priv)) {
			if (priv in PRIV) return [PRIV[priv]]
			else if (priv in PRIV_LUT) return [priv]
	...
	const handle = Object.assign(withSession(
		// Do the check
		conditional(wrap(async (req, res) => {
		...
		reduceCond(fn) {
			reduceCond = wrap(fn, fn.name || 'customReduceCond')
			return handle
		},
		denyAction(fn) {
			denyAction = wrap(fn, fn.name || 'customDenyAction')
			return handle
		},
	})
	return handle
