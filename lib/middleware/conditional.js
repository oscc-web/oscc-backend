...
export default function conditional(cond, ...servers) {
	...
	if (typeof cond !== 'function') throw new TypeError(`Illegal input type for cond: ${cond}`)
	...
	const handle = Object.assign(wrap(async function(req, res, next) {
		...
		if (!result) return onCondMismatch.handle(req, res, next)
		// Apply additional parameters to Request instance
		if (result && typeof result === 'object') Object.assign(req, result)
		// Pass this request to specified servers
		if (server) server.handle(req, res, next)
		else next()
	}, `conditional[${cond.name}]`), {
		/**
		 * Servers call upon condition failure
