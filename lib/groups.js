...
export default class Group extends CustomObject {
	...
	async challenge(user) {
		return await Group.challenge(this, user)
	}
	/**
	...
	static async locate(arg) {
		...
		arg = await Promise.all(
			arg
				...
				.map(async id => await SystemGroup.locate(id) || id)
		)
	 ...
	 * The challenge result
	 */
	static async challenge({ privileges = [] }, user) {
		for (const priv of privileges) {
			if (!await user.hasPriv(priv)) return false
		}
		return true
	}
	/**
