 ...
 * @property {'NONE' | 'SELF' | 'SAME-GROUP' | 'ALL'} visibility
...
export default class Group extends CustomObject {
	...
	async update(updateFilter) {
		if (updateFilter && typeof updateFilter === 'object') {
			...
			const [result] = await Group.db.find({ _id: this.id }).toArray()
			...
			return await Group.db.insert(groupDsc)
		}
	}
	/**
	...
	static async locate(arg) {
		...
		if (typeof arg === 'string') {
			...
			const [groupDsc] = await Group.db.find({ _id: id }).toArray()
		...
		get root() {
			return Group.db
				...
				.then(([{ localeName, visibility = 'SAME-GROUP' } = {}]) =>
					new class RootSystemGroup extends SystemGroup {
						constructor() {
