// Cluster instance initialization
...
export const PID = (
	extractIdentityFromURL(process.argv[1]) || 'untitled'
) + (
	cluster.isWorker ? `[${process.env.CLUSTER_ID || '*'}]` : ''
)
const info = [
...
export function extractIdentityFromURL(url) {
	if (process.env?.PID) return process.env?.PID
	if (url && typeof url === 'string') {
		const index = url.indexOf(PROJECT_ROOT)
